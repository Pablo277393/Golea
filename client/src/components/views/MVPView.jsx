          ...
          <div className="grid gap-4">
            {candidates.map((candidate, i) => {
              ...
              return (
                <div
                  key={candidate.id}
                  className={`relative p-6 rounded-2xl flex items-center justify-between border transition-all duration-500 group ${isActive
                    ? 'bg-primary/10 border-primary shadow-gold-glow'
                    : 'bg-white/5 border-white/5 hover:border-white/10 hover:bg-white/[0.08]'
                    }`}
                >
                  <div className="flex items-center gap-6">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg border transition-all ${isActive
